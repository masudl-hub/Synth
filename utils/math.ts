...
function lerpNoise(t: number, a: number, b: number) {
  return a + t * (b - a);
}
...
export const noise = (x: number, y: number, z: number) => {
  ...
  return lerpNoise(
    ...
    lerpNoise(
      v,
      lerpNoise(u, grad(perm[AA], x, y, z), grad(perm[BA], x - 1, y, z)),
      lerpNoise(u, grad(perm[AB], x, y - 1, z), grad(perm[BB], x - 1, y - 1, z))
    ),
    lerpNoise(
      v,
      lerpNoise(u, grad(perm[AA + 1], x, y, z - 1), grad(perm[BA + 1], x - 1, y, z - 1)),
      lerpNoise(u, grad(perm[AB + 1], x, y - 1, z - 1), grad(perm[BB + 1], x - 1, y - 1, z - 1))
    )
  );
};
